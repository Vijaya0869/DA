...
The shared dashboard `Header` (`apps/ui/src/components/Header/Header.tsx`, appears at the top of every logged-in page via `main/src/components/layout.tsx`) used to have a purely decorative search input — no `value`, no `onChange`, did nothing. Now wired to navigate to `/property?search=<term>` on Enter, reusing `PropertyList.tsx`'s already-working property search rather than building a second one. `PropertyList.tsx` reads `?search=` via `useSearchParams` — note it needs a `useEffect` watching `searchParams`, not just a `useState` initializer, since React Router doesn't remount the component on a same-route query-string-only navigation (the initializer alone silently misses the update). Fixing this also surfaced (and fixed) a real, separate bug: `property.service.ts`'s `applySearch` lowercased the search term but not the DB column, so `LIKE` (case-sensitive in Postgres) silently returned zero matches for anything not typed in the exact case stored in the DB — changed to `ILIKE`.
